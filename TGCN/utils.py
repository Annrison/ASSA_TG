...
def load_corpus(dataset_str):
    ...
    for i in range(len(names)):
        with open("{}.{}".format(dataset_str, names[i]), 'rb') as f:        
            # tocheck (textGCN 沒有encoding='latin1')
    ...
    print("length of val_size",val_size)
    ...
    return adj, adj1, adj2, features, y_train, y_val, y_test, train_mask, val_mask, test_mask, train_size, val_size,test_size
...
def construct_feed_dict(features, support, support_mix, labels, labels_mask, placeholders):
    ...
    feed_dict.update({placeholders['support_mix'][i]: support_mix[i]
                      for i in range(len(support_mix))})
...
def clean_str(string):
    ...
    string = re.sub(r"\$+", " $ ", string)
    string = re.sub(r"rrb", " ", string) # extra part
