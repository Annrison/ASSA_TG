...
if __name__ == "__main__":
    ...
    with open(f'./seed/sen_seed_{sys.argv[8]}.txt') as f:
        lines = f.read()    
    seeds = lines.split("|")
    train_pos_word = seeds[0].split(" ")
    train_neg_word = seeds[1].split(" ")
    ...
    sdir = './model_result/{0}'.format(sys.argv[5])
    ...
    for asp_name in asp_name_list:
        ...
        for i in range(10, len(oup)):
            if i < len(vocab_list):
                ...
                try:
                    if (word_freq[vocab_list[i]] >= freq_threshold) and (len(vocab_list[i])>2) and (":" not in vocab_list[i]) and (vocab_list[i] not in all_train_word):                    
